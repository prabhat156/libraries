nv.models.timelineAndSentimentChart = function() {
  ...
  function chart(selection) {
    selection.each(function(data) {
      ...
      //var testMarkerList = timelineData.filter(function(d,i){ return i==0}).map(function(series, i){ 
     ...
     */
      ...
      //WORK-BASIC//var markers = markerEnter.select('.nv-markerWrap').selectAll('g')
      //WORK-BASIC//                      .data(testMarkerList
      //WORK-BASIC//                              .filter(function(d){return !d.disabled})
      //WORK-BASIC//                              .map(function(d,i){
      //WORK-BASIC//                                  return {
      //WORK-BASIC//                                      key: d.key,
      //WORK-BASIC//                                      values : d.values.filter(function(d,i){
      //WORK-BASIC//                                          return lines.x()(d,i) >= extent[0] && lines.x()(d,i) <= extent[1];
      //WORK-BASIC//                                      })
      //WORK-BASIC//                                  }
      //WORK-BASIC//                              })
      //WORK-BASIC//                              )
      //WORK-BASIC//                  .enter()
      //WORK-BASIC//                      .append('g')
      //WORK-BASIC//                      .each(createMarkers);
     
      markerEnter.select('.nv-markerWrap').selectAll('g').remove();
      var markers = markerEnter.select('.nv-markerWrap')
                            .datum(testMarkerList
                                    .filter(function(d){return !d.disabled})
                                    .map(function(d,i){
                                        return {
                                            key: d.key,
      ...
      markers.transition().duration(transitionDuration).call(createMarkers_new);
       //TESTING MARKERS END
